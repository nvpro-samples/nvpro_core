...
void CameraManipulator::orbit(float dx, float dy, bool invert)
{
  ...
  rot_y = glm::rotate(glm::mat4(1), -dx, m_current.up);
  ...
  rot_x           = glm::rotate(glm::mat4(1), -dy, axe_x);
...
//
void CameraManipulator::fit(const glm::vec3& boxMin, const glm::vec3& boxMax, bool instantFit /*= true*/, bool tight /*=false*/, float aspect /*=1.0f*/)
{
  const glm::vec3 boxHalfSize = (boxMax - boxMin) * .5f;
  const glm::vec3 boxCenter   = boxMin + boxHalfSize;
  ...
  {
    glm::mat4 mView = glm::lookAt(m_current.eye, boxCenter, m_current.up);
    mView[3]        = glm::vec4(0, 0, 0, 1);

    for(int i = 0; i < 8; i++)
    {
      glm::vec3 vct(i & 1 ? boxHalfSize.x : -boxHalfSize.x, i & 2 ? boxHalfSize.y : -boxHalfSize.y,
                    i & 4 ? boxHalfSize.z : -boxHalfSize.z);
      vct = glm::vec3(mView * glm::vec4(vct, 1));

      if(vct.z < 0)  // Take only points in front of the center
      {
        // Keep the largest offset to see that vertex
        offset = std::max(fabs(vct.y) / tan(glm::radians(yfov * 0.5f)) + fabs(vct.z), offset);
        offset = std::max(fabs(vct.x) / tan(glm::radians(xfov * 0.5f)) + fabs(vct.z), offset);
      }
    }
  }

  // Re-position the camera
  auto viewDir = glm::normalize(m_current.eye - m_current.ctr);
